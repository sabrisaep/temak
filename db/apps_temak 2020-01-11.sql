...
--
-- Host: 127.0.0.1
-- Generation Time: Jan 11, 2020 at 12:57 PM
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `pensyarah`  AS  select `ptss_staf`.`senarai`.`idsenarai` AS `idpensyarah`,`ptss_staf`.`senarai`.`bahagian` AS `jabatan`,`ptss_staf`.`senarai`.`nokpstaf` AS `nokp`,`ptss_staf`.`senarai`.`namastaf` AS `namapensyarah`,`ptss_staf`.`senarai`.`kata` AS `kata` from `ptss_staf`.`senarai` where `ptss_staf`.`senarai`.`bahagian` in (select `jabatan`.`idjabatan` from `jabatan`) ;
